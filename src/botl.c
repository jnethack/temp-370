...
do_statusline1(void)
{
    ...
    newbot1[BOTL_NSIZ] = 0;
#else
    if (strlen(newbot1) > BOTL_NSIZ) {
        int n = offset_in_kanji((const unsigned char *)newbot1, BOTL_NSIZ);
        newbot1[BOTL_NSIZ - n] = 0;
    }
#endif
#if 0 /*JP*/
...
#endif
        {
            int cut = max(i, BOTL_NSIZ);
            if ((int) strlen(nb) > cut) {
                int n = offset_in_kanji(nb, cut);
                nb[cut - n] = '\0';
            }
        }
    }
