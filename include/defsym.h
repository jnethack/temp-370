...
#define PCHAR2(idx, ch, sym, tilenm, desc, clr) PCHAR(idx, ch, sym, desc, clr)
    ...
    PCHAR2(16, '+',  S_hcdoor, "horizontal closed door",
                               "閉じた扉", CLR_BROWN)
    PCHAR( 17, '#',  S_bars,   "鉄棒", HI_METAL)
    PCHAR( 18, '#',  S_tree,   "木", CLR_GREEN)
    PCHAR( 19, '.',  S_room,   "部屋の床", CLR_GRAY)
    PCHAR( 20, '.',  S_darkroom, "部屋の暗い部分", CLR_BLACK)
    PCHAR2(21, '`',  S_engroom, "engraving in a room", "engraving",
                                CLR_BRIGHT_BLUE)
    PCHAR2(22, '#',  S_corr,   "dark corridor", "通路", CLR_GRAY)
    PCHAR( 23, '#',  S_litcorr, "明るい通路", CLR_GRAY)
    ...
    /* end cmap A */
    PCHAR( 33, '_',  S_altar,  "祭壇", CLR_GRAY)
    /* start cmap B */
    PCHAR( 34, '|',  S_grave,  "墓", CLR_WHITE)
    PCHAR2(35, '\\', S_throne, "throne", "華やかな玉座", HI_GOLD)
    PCHAR( 36, '{',  S_sink,   "流し台", CLR_WHITE)
    PCHAR( 37, '{',  S_fountain, "泉", CLR_BRIGHT_BLUE)
    /* the S_pool symbol is used for both POOL terrain and MOAT terrain */
    PCHAR2(38, '}',  S_pool,   "pool", "水のたまった場所", CLR_BLUE)
    PCHAR( 39, '.',  S_ice,    "氷", CLR_CYAN)
    PCHAR( 40, '}',  S_lava,   "溶岩", CLR_RED)
    ...
    PCHAR2(45, '#',  S_hcdbridge, "horizontal closed drawbridge",
                                  "上っている跳ね橋", CLR_BROWN)
    PCHAR( 46, ' ',  S_air,    "空気のたまった場所", CLR_CYAN)
    PCHAR( 47, '#',  S_cloud,  "雲", CLR_GRAY)
    ...
    /* begin traps                                                     */
    /*                                                                 */
    PCHAR( 49, '^',  S_arrow_trap, "矢の罠", HI_METAL)
    PCHAR( 50, '^',  S_dart_trap, "投げ矢の罠", HI_METAL)
    PCHAR( 51, '^',  S_falling_rock_trap, "落石の罠", CLR_GRAY)
    PCHAR( 52, '^',  S_squeaky_board, "きしむ板", CLR_BROWN)
    PCHAR( 53, '^',  S_bear_trap, "熊の罠", HI_METAL)
    PCHAR( 54, '^',  S_land_mine, "地雷", CLR_RED)
    PCHAR( 55, '^',  S_rolling_boulder_trap, "巨岩の罠", CLR_GRAY)
    PCHAR( 56, '^',  S_sleeping_gas_trap, "睡眠ガスの罠", HI_ZAP)
    PCHAR( 57, '^',  S_rust_trap, "腐食の罠", CLR_BLUE)
    PCHAR( 58, '^',  S_fire_trap, "火の罠", CLR_ORANGE)
    PCHAR( 59, '^',  S_pit, "落し穴", CLR_BLACK)
    PCHAR( 60, '^',  S_spiked_pit, "トゲだらけの落し穴", CLR_BLACK)
    PCHAR( 61, '^',  S_hole, "穴", CLR_BROWN)
    PCHAR( 62, '^',  S_trap_door, "落し扉", CLR_BROWN)
    PCHAR( 63, '^',  S_teleportation_trap, "瞬間移動の罠", CLR_MAGENTA)
    PCHAR( 64, '^',  S_level_teleporter, "別の階への瞬間移動の罠", CLR_MAGENTA)
    PCHAR( 65, '^',  S_magic_portal, "魔法の入口", CLR_BRIGHT_MAGENTA)
    PCHAR( 66, '"',  S_web, "くもの巣", CLR_GRAY)
    PCHAR( 67, '^',  S_statue_trap, "彫像の罠", CLR_GRAY)
    PCHAR( 68, '^',  S_magic_trap, "魔法の罠", HI_ZAP)
    PCHAR2(69, '^',  S_anti_magic_trap, "anti magic trap", "反魔法空間の罠",
                                        HI_ZAP)
    PCHAR( 70, '^',  S_polymorph_trap, "変化の罠", CLR_BRIGHT_GREEN)
    PCHAR( 71, '~',  S_vibrating_square, "振動している場所", CLR_MAGENTA)
    ...
    PCHAR( 86, '#',  S_poisoncloud, "毒雲", CLR_BRIGHT_GREEN)
    ...
    PCHAR( 87, '$',  S_goodpos, "有効な位置", HI_ZAP)
    /* end cmap C */
    /*                                                             */
