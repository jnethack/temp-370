...
#error VIRTUAL_TERMINAL_SEQUENCESが必要
#endif
#endif
...
xputc2(const unsigned char *str)
{
    ...
    console.cursor.X = ttyDisplay->curx - (len - 1);
    console.cursor.Y = ttyDisplay->cury;

    /* 実際に何バイトでも移動するのは 2 バイト分 */
    ttyDisplay->curx -= len - 2;
...
VA_DECL(const char *, fmt)
{
    ...
    if (redirect_stdout || program_state.early_options)
        ...
        }
        {
            char *str = buf;
            while(*str){
                jbuffer(*(str++), NULL, (void (__cdecl *)(unsigned int))xputc_core, (void (__cdecl *)(unsigned char *))xputc2_core);
            }
        }
#endif
        if (ttyDisplay)
