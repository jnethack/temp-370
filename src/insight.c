...
*/
    ...
    if (u.uswallow) {
        ...
        Snprintf(buf, sizeof buf, "%s by %s",
                ...
                heldmon);
#else
        Snprintf(buf, sizeof buf, "%sに%s込まれて",
                heldmon,
...
*/
    item_resistance_message(AD_FIRE, "火からの保護", final);
    if (Cold_resistance)
/*JP
...
*/
    item_resistance_message(AD_COLD, "寒さからの保護", final);
    if (Sleep_resistance)
/*JP
...
*/
    item_resistance_message(AD_DISN, "粉砕からの保護", final);
    if (Shock_resistance)
/*JP
...
*/
    ...
    item_resistance_message(AD_ELEC, " protected from electric shocks",
                            final);
#else
    item_resistance_message(AD_ELEC, "電撃からの保護",
                            final);
#endif
...
*/
    item_resistance_message(AD_ACID, "酸からの保護", final);
    if (Drain_resistance)
/*JP
...
#else
        Sprintf(buf, " キャラクター再作成は%s回行われました．",
                N_times(u.uroleplay.numrerolls, bufN));
#endif
    enlght_out(buf);
